...
export function GetFreshestWorkListsStateFirstRow() {
  const allWorkListsState = useSelector((state) => state.WorkListsState.allRows);

  if (allWorkListsState != null && allWorkListsState.length > 0) {
    return allWorkListsState[0];
  } else {
    return allWorkListsState;
  }
}
