 ...
 *
 */
export const TestSubpageDB = ({ answers, route }) => {
  ...
  SqliteReduxTestPage.UpdateSpecificRowsFromDB({
    ...
    onSuccess: (row) => {
      console.log("item modifé avec succès dans TestSubpage.");

      GoToTestPageList();
    },
    onError: (e) => {
      console.log(
        "Un problème est survenu durant la modif d'item dans TestSubpage."
      );
